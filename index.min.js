...
LinkMatrixController = function($scope, $state, Schemas, Transformators, Graph) {
  ...
  return $scope.generateSchemas = function() {
    var cTotal, helper, iTotal, kTotal, sourceLinks, treeV, wConsumerTotal, wDiffTotal, wTotal, zTotal;
    ...
    while (true) {
      ...
      _.each(treeV, function(a) {
        ...
        return $scope.fullLinks.push({
          ...
          linkLength: $scope.source.linksLength[target.id],
          ip: target.ip,
    ...
    kTotal = _.sumBy($scope.fullLinks, 'k');
    iTotal = _.sumBy($scope.fullLinks, 'i');
    wDiffTotal = _.sumBy($scope.fullLinks, 'wDiff');
    wConsumerTotal = _.sumBy(_.map($scope.fullLinks, 'consumer'), 'w');
    wTotal = 100 * wDiffTotal / wConsumerTotal;
    cTotal = 1.9 * wTotal;
    zTotal = 0.2 * kTotal + iTotal;
    $scope.fullLinks.push({
      linkLength: _.sumBy($scope.fullLinks, 'linkLength'),
      k: kTotal,
      i: iTotal,
      wDiff: wDiffTotal,
      w: wTotal,
