...
LinkMatrixController = function($scope, $state, Schemas, Transformators, Graph) {
  ...
  return $scope.generateSchemas = function() {
    ...
    _.each($scope.fullLinks, function(link) {
      ...
      link.p = 6.3;
      return link.i = link.p / 100 * link.k;
    });
    $scope.fullLinks.push({
      linkLength: _.sumBy($scope.fullLinks, 'linkLength'),
      k: _.sumBy($scope.fullLinks, 'k'),
      i: _.sumBy($scope.fullLinks, 'k')
    });
    return drawGraph();
  };
};
...
angular.module('transformators').factory('Transformators', function() {
  var countQuarterTotalPower, countTotalPower, findTransformator, getJ, getLine, getLineCost, getSource, getTargets, lineCostList, linesList, maxPower, setTargets, source, targets, transformatorList, transformatorTargetList;
  maxPower = 15000;
  targets = [
    {
      x: 3,
